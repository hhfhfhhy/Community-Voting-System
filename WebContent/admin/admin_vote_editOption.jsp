...
function check_NotNull(){
	var OptionContent = document.getElementById("OptionContent").value;
	var strOptionContent=OptionContent.replace(/(^\s*)|(\s*$)/g,'');
	
	if(OptionContent==""||OptionContent==null||strOptionContent==""){
		alert("选项不能为空！！！！");
		return false;
	}
...
<form action="/OVS/EditOptionServlet" method="post" onsubmit="return check_NotNull()">
			<table>
				<tr>
					<td>选项：</td>
					<td>&nbsp;</td>
				...
				<%
				//if(currentVoteEvent==null){out.println("<td><p style=\"color:red\"><big>暂无选项。点击<a href=\"admin_vote_addOption.jsp?VoteID="+VoteID+"&AdminID="+admcurrentUser.getAdminID()+"\">添加</a>。</big></p></td>");}
				
					for(int i=0;i<currentVoteEvent.size();i++){
						//out.println("<tr><td>"+currentVoteEvent.get(i).getOptionSequence()+"</td>");
						out.println("<tr><td>&nbsp;</td>");
						//out.println(OptionSequence);
						if(currentVoteEvent.get(i).getOptionSequence().equals(OptionSequence)){
							int j=i;
							out.println("<td><input type=\"text\" id=\"OptionContent\" name=\"OptionContent\" value=\""+currentVoteEvent.get(j).getOptionContent()+"\"/></td>");
						}else{							
				...
				<tr>
					...
					<td><input type="hidden" name="OptionSequence" value="<%=OptionSequence%>"/></td>
				</tr>
			</table>
			</form>
		</fieldset>
